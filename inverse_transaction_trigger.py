...
class InverseTransactionTrigger(TransactionBase):
    ...
    def _is_up_market(self):        
        return ((self.currency_client.snapshot <= (self.lower_bound * -1)) and (self.currency_client.snapshot >= (self.upper_bound * -1)) and (self.currency_client.macd_diff < 0) and (self.currency_client.ema_diff < self.ema_diff_limit) and (self.currency_client.longterm < 0))
