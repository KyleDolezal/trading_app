...
class TransactionTrigger(TransactionBase):
    ...
    def _is_up_market(self):
        return ((self.currency_client.longterm >= self.lower_bound) and (self.currency_client.longterm <= self.upper_bound) and (self.currency_client.macd_diff > 0) and (self.currency_client.ema_diff > 0) and (self.currency_client.ema_diff < self.ema_diff_limit) and (self.currency_client.snapshot > 0))